...
"""
Create CMS 2012 configuration file records.
"""
...
def get_run_period(afile):
    ...
    return 'Run2012A-Run2012D'  # FIXME
...
def main():
    ...
    recid = RECID_START
    fdesc = open('./outputs/config_files_link_info.py', 'w')
    ...
    for root, dirs, files in os.walk('./inputs/config-store'):
        for afile in files:
            ...
            rec['cms_confdb_id'] = os.path.basename(afile).replace('.configFile', '')
