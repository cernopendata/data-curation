...
"""
Create CMS 2013 HI ppref collision dataset records.
"""
...
recid_freerange_start = 24640
recid_validated_runs_full_validation = 14218
recid_validated_runs_muons_only = 14219
...
collision_type = "pp"
year_published = "2023"
run_number_min = "211739"
run_number_max = "211831"
...
def populate_fwyzard():
    ...
    if os.path.exists("./inputs/hlt-2013-hi-ppref-datasets.txt"):
        for line in open("./inputs/hlt-2013-hi-ppref-datasets.txt", "r").readlines():
            line = line.strip()
...
def create_selection_information(dataset, dataset_full_name):
    ...
    for trigger_path in trigger_paths:
        ...
        out += f'<br/><a href="/search?q=%s&type=Supplementaries&year=2013">%s</a>' % (
            trigger_path,
            trigger_path,
        )
    out += "</p>"
...
def create_record(recid, dataset_full_name):
    ...
    rec["abstract"] = {}
    abstract_description = f"<p>{dataset_short} primary dataset in {dataset_format} format from {dataset_runperiod_runx} of {dataset_runperiod_year}."
    abstract_description += (
        f" Run period from run number {run_number_min} to {run_number_max}.</p>"
    )
    if dataset_full_name == "/PPJet/Run2013A-HighPtPP-15Feb2013-v1/RECO":
        abstract_description += (
            "<p>This dataset is a skim of the related dataset linked below.</p>"
        )
    if dataset_full_name == "/PPJet/Run2013A-PromptReco-v1/RECO":
        abstract_description += (
            "<p>This dataset is related to the skim linked below.</p>"
        )
    abstract_description += "<p>These proton-proton data were taken at the same centre-of-mass energy and have a similar trigger menu to Pb-Pb collision data from 2010/2011.</p>"
...
def enrich_records_with_sibling_links(records):
    """Enrich generated records with sibling links between PromptReco and 15Feb2013."""
    for record in records:
        if "PromptReco" in record["title"]:
            this_dataset = "PromptReco-v1"
            other_dataset = "HighPtPP-15Feb2013-v1"
        elif "HighPtPP-15Feb2013-v1" in record["title"]:
            this_dataset = "HighPtPP-15Feb2013-v1"
...
def main():
    ...
    with open("./inputs/cms-2013-collision-datasets-hi-ppref.txt", "r") as f:
        dataset_full_names = []
