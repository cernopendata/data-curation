...
with open('ComputingNotes.txt', 'r') as f:
...
namelist, authorlist, datelist = np.array(sorted(list(zip(namelist, authorlist, datelist)))).T
...
for name, authors, file, date in zip(namelist, authorlist, fileinfo, datelist):
    ...
    entry = {'abstract': {'description': fields[1]},
    ...
    "distribution": {
        ...
        "size": int(size)},
    'experiment': ['JADE'],
    'files': [{'checksum': 'adler32:'+ checksum,
        'size': int(size),
...
with open('jade-computing-notes.json', 'w', encoding='utf-8') as f:
