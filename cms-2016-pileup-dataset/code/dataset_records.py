...
def create_record(dataset_full_name, doi_info, recid_info, eos_dir, das_dir, mcm_dir, conffiles_dir):
    ...
    rec['abstract'] = {}
    rec['abstract']['description'] = '<p>' + additional_title + '. Events are sampled from this dataset and added to simulated data to make them comparable with the ' + \
            year_created + \
            ' collision data, see <a href="/docs/cms-guide-pileup-simulation">the guide to pile-up simulation</a>.</p>' + \
            ...
            '<p>This is a sample (10%) of the original dataset.</p>'
    ...
    if pileup_dataset_name:
        ...
        if pileup_dataset_recid:
            ...
            rec['pileup']['description'] = "<p>To make these simulated data comparable with the collision data, <a href=\"/docs/cms-guide-pileup-simulation\">pile-up events</a> from the dataset <code>"\
                                            + pileup_dataset_name\
                                            + "</code> are added to the simulated event in the DIGI step.</p>"

    pusteps = str(get_all_generator_text(pileup_dataset_name, das_dir, mcm_dir, conffiles_dir, recid_info))[:-2]
    dsstep = str(get_all_generator_text(dataset_full_name, das_dir, mcm_dir, conffiles_dir, recid_info))
    a,b = dsstep.split("'steps': [", 1)
    allsteps = pusteps + "," + b

    rec['methodology'] = literal_eval(allsteps)
