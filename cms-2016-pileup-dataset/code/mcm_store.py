...
def mcm_downloader(dataset, mcm_dir):
    ...
    print("in mcm_dowloader for " + dataset + mcm_dir) 
    ...
    print("Decoded chain prepid " + mcm_chain_prepid_out)
    chain_prepid = re.sub(r"[\[\]]", "", mcm_chain_prepid_out).strip()
    print("Using chain prepid " + chain_prepid)
    # if two parts, take first
    if "," in chain_prepid:
        a,b = chain_prepid.split(',', 1)
        print(a)
        print(b)
        chain_prepid = a
...
def create(datasets, mcm_dir, eos_dir, ignore_eos_store=False):
    ...
    for dataset in eos_datasets:
        print("McM Storing ({i}/{N}) {ds}".format(i=i, N=total, ds=dataset))
        mcm_downloader(dataset, mcm_dir)
...
def get_pileup_from_mcm(dataset, mcm_dir):
    ...
    for step_dir in step_dirs:
        ...
        if m:
            # FIXME: updated for the pileup from DIGIPremix to Premix, check if it breaks for the normal MC
            step_name = m.group(1)
            if step_name.endswith('Premix'):
