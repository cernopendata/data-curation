...
"""
Create 2013 MC HI dataset records.
"""
...
from mcm_store import (get_cmsDriver_script, get_cmssw_version_from_mcm,
                       get_conffile_ids_from_mcm, get_dataset_collision_energy,
                       get_dataset_collision_type,
...
def get_dataset_index_files(dataset_full_name, eos_dir):
    ...
    for line in output.split('\n'):
        ...
        if afile.endswith('.txt') or afile.endswith('.json'):
            ...
            afile_size = get_file_size(eos_dir + os.sep + afile)
            # FIXME CHANGE THIS checksum TO READ FROM SOMETHING SOMWEHERE
            afile_checksum = '{:#010x}'.format(get_file_checksum(eos_dir + os.sep + afile)).split('0x')[1]
...
def create_record(dataset_full_name, doi_info, recid_info, eos_dir, das_dir, mcm_dir, conffiles_dir):
    ...
    year_published = '2023'  # FIXME get from somewhere, do not hardcode it!
    run_period = ['Run2013']  # FIXME Hardcoded!!
    ...
    collision_energy = get_dataset_collision_energy(dataset_full_name, mcm_dir)
    collision_type = get_dataset_collision_type(dataset_full_name, mcm_dir)

    description_addon = ''
    if collision_type == 'pp':
        description_addon = ', needed as reference data for heavy-ion data analysis,'
    description_year = 'FIXME'
    if collision_type == 'PbPb' and collision_energy == '2.76TeV':
        description_year = 'during Run1'
    elif collision_type == 'pPb':
        description_year = 'in 2013'
    elif collision_type == 'pp':
        description_year = 'in 2013'
    else:
        description_year = 'in FIXME'
    ...
    rec['abstract']['description'] = '<p>' + additional_title + '.</p>' + \
                                     ...
                                     f'<p>These simulated datasets correspond to the {collision_type} collision data{description_addon} at energy {collision_energy} collected by the CMS experiment {description_year}.</p>'
    ...
    rec['categories'] = {}
    rec['categories']['primary'] = ' Heavy-Ion Physics'
...
def main(datasets, eos_dir, das_dir, mcm_dir, conffiles_dir, doi_file, recid_file):
    ...
    records = create_records(datasets, doi_file, recid_file, eos_dir, das_dir, mcm_dir, conffiles_dir, records_dir)
