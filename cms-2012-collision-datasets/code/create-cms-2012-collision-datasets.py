...
def populate_selection_descriptions():
    ...
    for input_file in [
        "./inputs/CMSDatasetDescription_Run2012B.csv",
...
def get_dataset_files(dataset, run_period, version):
    ...
    for filepath in FILEINFO.keys():
        if filepath.startswith(
            "/eos/opendata/cms/" + run_period + "/" + dataset + "/AOD/" + version + "/"
        ):
...
def create_record(recid, run_period, dataset):
    ...
    if run_period == "Run2012B":
        rec["abstract"]["description"] = (
            "<p>"
            + dataset
            + " primary dataset in AOD format from RunB of 2012. Run period from run number 193833 to 196531.</p>"
            + " <p>This dataset contains all runs from 2012 RunB. The list of validated runs, which must be applied to all analyses, can be found in</p>"
        )
        rec["abstract"]["links"] = [
    ...
    if run_period == "Run2012C":
        rec["abstract"]["description"] = (
            "<p>"
            + dataset
            + " primary dataset in AOD format from RunC of 2012. Run period from run number 198022 to 203742.</p>"
            + " <p>This dataset contains all runs from 2012 RunC. The list of validated runs, which must be applied to all analyses, can be found in</p>"
        )
        rec["abstract"]["links"] = [
            {"recid": recid_validated_runs},
        ]
    ...
    for index_type in [".json", ".txt"]:
        ...
        for file_number, (file_uri, file_size, file_checksum) in enumerate(index_files):
            rec["files"].append(
                {
                    "checksum": "adler32:" + str(file_checksum),
                    "description": dataset
