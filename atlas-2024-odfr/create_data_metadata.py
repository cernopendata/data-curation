...
dataset_input = 'pp_data_p6026_GRL.txt'
...
for dataset_number,dataset in enumerate(datasets):
    ...
    # Grab the list of files from rucio - for data, we are always going to take _all_ the events
    ...
    for a in fl:
        ...
        file_locations[dataset][ my_scope+':'+a['name'] ] = { 'checksum':'adler32'+a['adler32'], 'size':a['bytes'], 'events':a['events'], 'type':'DAOD_PHYSLITE' }
...
import json
with open( 'data_file_mapping'+static_did_post+'.json' , 'w' ) as file_backup:
