...
MARC_FILE = '../../opendata.cern.ch/invenio_opendata/testsuite/data/cms/cms-primary-datasets-Run2011A.xml'
...
def main():
    ...
    for line in open(MARC_FILE, 'r').readlines():
        if '</record>' in line:
            ...
            match = re.match(r'^\s+<subfield code="a">(/.*AOD)</subfield>\s+$', line)
            if match:
