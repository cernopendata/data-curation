...
def get_das_json(dataset):
    ...
    if os.path.isfile(filepath):
        ...
        return json.loads(subprocess.check_output(["python", "./das.py",
                                                   "--query",
                                                   "dataset="+dataset,
                                                   "--format=json"]))
...
def get_input_dataset(das):
    ...
    input_datasets = get_from_deep_json(das, 'InputDatasets')
    if input_datasets and isinstance(input_datasets, list):
...
def main():
    """Do the job."""
    for title in open('./inputs/cms-primary-datasets-Run2011A.txt', 'rb').readlines():
