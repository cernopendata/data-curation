...
do
    ...
    mkdir -p $TARGETDIR/${STRIPPINGVERSION}_html

    echo Creating index...
    
    python3 IndexPageStripper.py --inputfile $SOURCEDIR/$STRIPPINGVERSION/index.html --outputfile $TARGETDIR/${STRIPPINGVERSION}_html/index.html

    python cleanIndexPages.py $TARGETDIR/${STRIPPINGVERSION}_html/index.html  $TARGETDIR/${STRIPPINGVERSION}_html/ $STRIPPINGVERSION;
    pandoc $TARGETDIR/${STRIPPINGVERSION}_html/index_clean.html  --from html --to gfm --lua-filter=links-to-md.lua | sed 's/\\\[/[/g' | sed 's/\\\]/]/g' > $TARGETDIR/$STRIPPINGVERSION/${STRIPPINGVERSION}_index.md;
    ...
    rm -r $TARGETDIR/${STRIPPINGVERSION}_html
done;
