...
for dataset in datasets:
    ...
    for arule in rc.list_did_rules(scope=my_scope, name=dataset):
        if arule['rse_expression'] == 'CERN-PROD_OPENDATA':
        ...
        if really_use_rucio:
            rid = rc.add_replication_rule(dids=[{ 'scope':my_scope, 'name':dataset }], # The list of datasets we want to transfer
...
print(f'Moved {total_so_far*1e-12} TB, with {total_to_move*1e-12} TB still to go')
print(f'That includes {count_so_far} datasets moved and {count_to_move} still to go')
