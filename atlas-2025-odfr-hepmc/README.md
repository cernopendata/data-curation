...
script.
* `EVNT_extra_datasets.txt` is a last catch for any additional datasets that should be
...
datasets (in json format). It reads input from `EVNT_list_*.txt`, `EVNT_extra_datasets.txt`, and
...
* `transfer_hepmc.py` is a script to transfer datasets to the CERN open data endpoint. It reads
...
* Run `build_full_metadata_json.py` to generate the database-ready json files to be uploaded to ATOM
* Create a merge request for all these scripts
* Tag the data as Open Data in AMI. For this, you might need to request rights from ami@lpsc.in2p3.fr:

```bash
for adid in `grep "^mc23" HEPMC_datasets.txt`; do ami command AddPhysicsParameterVals -scope=mc23 -paramName="openData" -paramValue="1" -propagate="0" -overwrite="0" -physicsGroup="ATOD" -logicalDatasetName="${adid}"; done
for adid in `grep "^mc15" HEPMC_datasets.txt`; do ami command AddPhysicsParameterVals -scope=mc15 -paramName="openData" -paramValue="1" -propagate="0" -overwrite="0" -physicsGroup="ATOD" -logicalDatasetName="${adid}"; done
```
...
* Run `transfer_hepmc.py` to transfer the (additional) HEPMC files to CERN
* Run `create_file_metadata.py` to generate a new `od_hepmc_file_mapping*.json`
